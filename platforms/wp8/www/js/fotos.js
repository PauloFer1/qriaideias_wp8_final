...
function checkConnection() {
    ...
    $('#page4').live('pageshow', function () {
        ...
        var url;
        if (userType == 4)
            url = URL_BO+'/projectos/mobilerequest/getimagescliente/' + idProjecto + '/' + idLoja + '&jsoncallback=?';
        else
            url = URL_BO+'/projectos/mobilerequest/getimages/' + idProjecto + '/' + idLoja + '&jsoncallback=?';
                      ...
                      if (!isOdd(i)) {
                          var li = '<div class="ui-block-a" class="imgDivGal">'+
                                      '<div class="divGal">'+html+
                                          '<div class="imgBorder">'+
                                              '<img id="' + idFoto + '" rel="' + foto + '" class="fotoList" src="'+URL_BO+'/public/images/' + foto + '" alt="' + fotoNome + '"/>'+
                                          '</div>'+
                                      '</div>'+
                          ...
                          var li = '<div class="ui-block-b" class="imgDivGal"><div class="divGal">'+html+'<div class="imgBorder"><img id="' + idFoto + '" rel="' + foto + '"  class="fotoList" src="'+URL_BO+'/public/images/' + foto + '" alt="' + fotoNome + '"/></div></div> <span class="fotoDescription" id="desc_'+idFoto+'">'+ descricao +'</span></div>';
                      }
                      $(".fotosList").append(li);
                  });
    ...
    $(document).on('click', '.fotoList', function () {
        ...
        url = url.replace(url.substring(url.lastIndexOf('/') + 1), "index.html#page5");
    ...
    $(document).on('click', '#btnInsertCommentMenu', function (e) {
        ...
        var url = window.location.href;
        url = url.split('#').pop().split('?').pop();
        url = url.replace(url.substring(url.lastIndexOf('/') + 1), "index.html#page11");
        $.mobile.changePage(url, { reloadPage: false, changeHash: false });
    });
    $(document).on('click', '#btnComment', function (e) {
        ...
        var url = window.location.href;
        url = url.split('#').pop().split('?').pop();
        url = url.replace(url.substring(url.lastIndexOf('/') + 1), "index.html#page10");
        $.mobile.changePage(url, { reloadPage: false, changeHash: false });
    });
    ...
    $('#page5').live('pageshow', function () {
        ...
        localStorage.setItem("history", "#page4");
        $('#imgBig').unbind('click');
        $('#content').hide();
        var nomeProjecto = localStorage.getItem('nomeProjecto');
        var nomeLoja = localStorage.getItem('nomeLoja');
        ...
        var url1 = null;
        url1 = URL_BO+'/public/images/' + nomeFoto;
        var url2 = URL_BO+'/projectos/mobilerequest/getimage/' + idFoto + '&jsoncallback=?';

        $("#imgBig").attr("src", url1);

         $.getJSON(URL_BO+'/projectos/mobilerequest/getimage/' + idFoto + '&jsoncallback=?', function(data)
          {
              if (data !== '')
              {
                  $.each(data, function(i, item)
    ...
    function editDescription(results) {
        if (results.buttonIndex == 1) {
            checkConnection();
            var url = URL_BO+'/projectos/mobilerequest/editdescricaofoto/' + localStorage.getItem('idFoto') + '/&jsoncallback=?';
                $.ajax({
                    type: 'POST',
                    scriptCharset: 'utf-8',
                    contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                    data: {descricao: results.input1},
                    url: URL_BO+'/projectos/mobilerequest/editdescricaofoto/' + localStorage.getItem('idFoto') + '/&jsoncallback=?',
    ...
    $('#page11').live('pageshow', function () {
        ...
        localStorage.setItem("history", "#page10");
        var nomeProjecto = localStorage.getItem('nomeProjecto');
        var nomeLoja = localStorage.getItem('nomeLoja');
    ...
    function insertCommentFoto(comment) {
        checkConnection();
        var url = URL_BO+'/projectos/mobilerequest/insertcommentfoto/' + localStorage.getItem('userId') + '/' + localStorage.getItem('idFoto') + '/&jsoncallback=?';
          $.ajax({
               type: 'POST',
               scriptCharset: 'utf-8',
               contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
               data: { comentario: comment },
               url: URL_BO+'/projectos/mobilerequest/insertcommentfoto/' + localStorage.getItem('userId') + '/' + localStorage.getItem('idFoto') + '/&jsoncallback=?',
               success: function (data) {
                   // console.log(data);
                   if (data == 1)
    ...
    function insertDescription(results) {
        checkConnection();
        var url = URL_BO+'/projectos/mobilerequest/insertdescricaofoto/' + localStorage.getItem('idFoto') + '/&jsoncallback=?';
          $.ajax({
              type: 'POST',
              scriptCharset: 'utf-8',
              contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
              data: {descricao: results.input1},
              url: URL_BO+'/projectos/mobilerequest/insertdescricaofoto/' + localStorage.getItem('idFoto') + '/&jsoncallback=?',
              success: function(data)
              {
                  // console.log(data);
                  if (data == 1)
    ...
    function confirmValidation(index) {
        ...
        if (index == 1) {
            ...
            var url = URL_BO+'/projectos/mobilerequest/validate_foto/' + id + '/' + newValida + '/&jsoncallback=?';
              $.ajax({
                   type: 'POST',
                   scriptCharset: 'utf-8',
                   contentType: 'application/x-www-form-urlencoded; charset=UTF-8',
                   url: URL_BO+'/projectos/mobilerequest/validate_foto/' + id + '/' + newValida + '/&jsoncallback=?',
                   success: function(data)
                   {
                       // console.log(data);
                       if (data == 1){
    ...
    function uploadPhoto(imageURI) {
        ...
        ft.upload(imageURI, URL_BO+"/projectos/mobilerequest/uploadfoto", win, fail, options);
    ...
    function win(r) {
        ...
        if (obj.success === true) {
            ...
            if (userType == 1)
                ...
                html = '';
            var li = '<div class="ui-block-a" class="imgDivGal"><div class="divGal">' + html + '<div class="imgBorder"><img id="' + obj.foto_id + '" rel="' + obj.image + '"  class="fotoList" src="' + URL_BO + '/public/images/' + obj.image + '" alt="' + obj.image + '"/></div></div><span class="fotoDescription" id="desc_' + obj.foto_id + '"></span></div>';
            $(".fotosList").append(li);
